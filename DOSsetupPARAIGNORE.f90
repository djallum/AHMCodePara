...
contains
  ...
  subroutine GetDos(my_DOS, my_droppedDOS, SitePotential, Teff, ClusterSize, weakL, weakR, SitesRemoved, SitesIgnored, Hops )
    implicit none
                ...
                !---------------------------Inputs-------------------------------------------
    real, dimension(:), intent(in) :: SitePotential, Hops
    ...
    if ( ClusterSize .eq. dim+1 ) then
       ...
       CALL DiagCluster(ClusterSize,4**ClusterSize,Sites,Cont,Weight,ChemPot,uSite,Hops)
...
!       CALL Bin_Data(DOS, Cont, Weight, DroppedDos, DOS_EMax, DOS_EMin)
  ...
  subroutine System_DoS( my_DOS, my_droppedDOS, SitePotential, Hopping, Bonds, WeakBonds, SitesIgnored )
    ...
    real, intent(inout) :: my_droppedDOS
...
!!$          end if
  ...
  subroutine Full_Dos()
    ...
    real :: my_DOS(bins,DOS_MaxCluster), my_droppedDOS
