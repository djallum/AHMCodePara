...
!    integer, dimension(nsites,2) :: neighbours           ! neighbours(i,:) is all the site that are nearest neighbours to site i
    ...
    PESdn_MBG=0.d0; PESup_MBG=0.d0; IPESdn_MBG=0.d0; IPESup_MBG=0.d0
    grand_potential = 0.d0
    e_ground = 0.d0
    Energy = 0.d0
    Weight = 0.d0
    ...
    g_up = groundloc(1) - 1
    g_dn = groundloc(2) - 1
    !print*, "cluster", g_up, g_dn, nsites
    min_up = MAX(0,g_up-1)             
    ...
    if ( (g_up .eq. 0) .and. (g_dn .eq. 0) ) then
 
       !do i=1,size(Energy)
        !  print*, Energy(i), Weight(i)
       !end do
